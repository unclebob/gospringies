...
A mass whose path segment from its previous position to its current timestep position crosses a wall spring segment must collide with that wall spring even when the mass's final position is already on the far side of the wall spring.
