...
func analyzeDrawnFrame(game *Game) DrawFrameReport {
	...
	for name := range visibleRegionRects() {
		report.RegionPixels[name] = visibleRegionPixels(name)
	}
...
func visibleRegionPixels(region string) int {
	count := 0
	for _, control := range visibleControls() {
	...
	if region == "status line" {
		for _, field := range (&Game{simulation: newDefaultStartupWorld()}).statusFields() {
