...
func (s *Simulation) applyMovingWallSpringFixedEndpointCollision(aIndex, bIndex, fixedIndex int, startPositions []Vec2) {
	if fixedIndex == aIndex || fixedIndex == bIndex || !s.Masses[fixedIndex].Fixed {
		return
	}
	endpointA := &s.Masses[aIndex]
	endpointB := &s.Masses[bIndex]
	fixed := s.Masses[fixedIndex]
	previousA := wallSpringPreviousPosition(*endpointA, startPositions, aIndex, 0)
	previousB := wallSpringPreviousPosition(*endpointB, startPositions, bIndex, 0)
	...
	currentSegment := endpointB.Position.Sub(endpointA.Position)
	currentLengthSquared := dot(currentSegment, currentSegment)
	if currentLengthSquared == 0 {
		return
	}
	currentFraction := closestFractionOnSegment(fixed.Position, endpointA.Position, currentSegment, currentLengthSquared)
	currentContact := endpointA.Position.Add(currentSegment.Scale(currentFraction))
	currentSide := dot(currentContact.Sub(fixed.Position), normal)
	if currentSide >= fixedMassCollisionRadius {
		return
	}
	oldVelocity := wallSpringContactVelocity(endpointA, endpointB, currentFraction)
	newVelocity := resolvedFixedEndpointContactVelocity(oldVelocity, normal)
	contactDelta := newVelocity.Sub(oldVelocity)
	if contactDelta == (Vec2{}) && currentSide >= 0 {
		return
	}
