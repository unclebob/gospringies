...
func TestGenerateTaggedGoTestAddsBuildTag(t *testing.T) {
	dir := t.TempDir()
	ir := filepath.Join(dir, "feature.json")
	output := filepath.Join(dir, "feature_acceptance_test.go")
	if err := os.WriteFile(ir, []byte(`{"name":"Tagged"}`), 0o644); err != nil {
		t.Fatal(err)
	}

	if err := GenerateTaggedGoTest(ir, output, "acceptance_mutation"); err != nil {
		t.Fatalf("GenerateTaggedGoTest returned error: %v", err)
	}
	content, err := os.ReadFile(output)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(content), "//go:build acceptance_mutation") {
